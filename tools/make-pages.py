...
# ============================================================
# /cases  導入事例
#   お客様名は出さない。数字はこれまでサイトで案内してきた実績の範囲にとどめ、
#   「よくあるご依頼の形」として、課題→用意したもの→結果が分かる形にする。
# ============================================================
CASES = [
    dict(tag='中小企業', title='新入社員30名ぶんのPCを、入社日までに用意する',
         ...
         result='入社日に合わせて納品し、<b>初期設定の社内工数はほぼゼロ</b>。'
                '追加入社が決まったぶんも、同じ機種を後から追加した。'),
    dict(tag='短期プロジェクト', title='3か月だけ使うPCを、増員に合わせて用意する',
         ...
         result='案件終了後は<b>返却日をそろえて一括回収</b>。データ消去証明書も発行し、'
                '資産として残さずに済んだ。'),
    dict(tag='オフィス開設', title='新しい拠点の立ち上げを、機器とネットワークごと任せる',
         ...
         result='<b>窓口を1つにまとめた</b>ことで、開設日に合わせて一度に立ち上げられた。'),
    dict(tag='研修・イベント', title='3日間のセミナー会場へ、PCを直送して当日設置する',
         ...
         result='<b>1日で設置完了</b>。研修終了後はそのまま回収したため、社内作業は発生しなかった。'),
    dict(tag='PC入替', title='古い機体から順に入れ替えて、データ消去証明まで残す',
         ...
         result='<b>業務を止めずに順番に入替</b>。処分の記録も残せた。'),
    dict(tag='教育・自治体', title='研修でPCとAI講座をまとめて用意する',
         ...
         result='<b>機器と学習環境を1つの契約で</b>用意でき、受講者の入れ替えにも対応できた。'),
]
...
def cases_body():
    ...
    <div class="in">
      <div class="crumb"><a href="/">COMMERCE</a>　›　導入事例</div>
      <h1>導入事例</h1>
      <p>どんな課題に、何を、何台、どのくらいの期間でご用意したのかをまとめています。
  ...
  <div class="c-sec">
    ...
    <div class="note-box">お客様名・写真は掲載していません。上記はご依頼の多い形を、実際の対応内容にもとづいて整理したものです。
      台数・期間・構成はご相談に応じて変わります。具体的な条件でのお見積りは、3分の無料診断からお送りください。</div>
  </div>
'''
...
def main():
    ...
    page('cases.html', '導入事例｜法人IT調達・レンタルの実例｜COMMERCE',
         '新入社員30名のPC調達、3か月の短期プロジェクト、新規オフィス開設、研修会場への直送、PC入替など、'
         '課題・台数・期間・設定内容・結果をまとめた導入事例です。',
         '/cases', cases_body())
